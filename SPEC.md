...
| `#back` | `<!-- #back -->` | 戻るリンク |
| `#ls` | `<!-- #ls -->` | 子ページ一覧 |
| `#ls2` | `<!-- #ls2 -->` または `$lsx(相対パス)` | 子ページ一覧（拡張版）<br>`--convert-ls2-to-lsx` オプションで GROWI lsx 形式に変換可能 |
| `#menu` | `<!-- #menu -->` | メニュー表示 |
...
`--convert-ls2-to-lsx` オプションを使用すると、PukiWiki の `#ls2` プラグインを GROWI の `$lsx` 形式に変換できます。
