...
**注意事項:**
- `#br` の前に空白やタブがある場合は整形済みテキストとして扱われます
- 行末の空白は変換前に削除されます
...
PukiWikiのテーブルは、`|h`（ヘッダー行マーカー）の有無によって出力形式が異なります：

1. **`|h`がある場合**: Markdown形式のテーブルとして出力
   - セパレーター行が自動挿入される
   - 列の配置指定（LEFT/CENTER/RIGHT）がサポートされる

2. **`|h`がない場合**: HTMLテーブル形式として出力
   - `<table>`, `<tr>`, `<td>`タグを使用
   - すべてのセルが`<td>`として出力される（`<th>`は使用しない）
   - Markdownテーブルでは最初の行が必ずヘッダーとして扱われるため、行指向のテーブルにはHTMLを使用
...
`~`と配置指定は組み合わせることができます（Markdown形式のみ）：
...
- 行頭に空白がある `----` や `#hr` は整形済みテキストとして扱われます（水平線にはなりません）
...
その他のプラグイン記法（`#contents`、`#ls`、`#include`、`#comment`、`#tracker`、`#calendar`など）はそのまま残します。
