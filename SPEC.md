...
プラグイン名（`#`や`&`で始まるもの）は、**大文字小文字を区別せず**に処理されます。これはPukiWikiの仕様に準拠しています。

**ブロックプラグイン（`#`で始まるもの）:**
- `#ref`, `#REF`, `#Ref`, `#rEf` などはすべて同じプラグインとして認識されます
- `#vote`, `#VOTE`, `#Vote` などはすべて同じプラグインとして認識されます
- `#include`, `#INCLUDE`, `#Include` などはすべて同じプラグインとして認識されます
- `#hr`, `#HR`, `#Hr` などはすべて同じプラグインとして認識されます
- `#br`, `#BR`, `#Br` などはすべて同じプラグインとして認識されます
- その他のすべてのブロックプラグインも同様です

**インラインプラグイン（`&`で始まるもの）:**
- `&br;`, `&BR;`, `&Br;` などはすべて同じプラグインとして認識されます
- `&size`, `&SIZE`, `&Size` などはすべて同じプラグインとして認識されます
- `&color`, `&COLOR`, `&Color` などはすべて同じプラグインとして認識されます
- `&ref`, `&REF`, `&Ref` などはすべて同じプラグインとして認識されます
...
出力:
![image.png](ページ名_attachment_image.png)
<!-- #Vote(選択肢1[0],選択肢2[1]) -->
| 選択肢 | 投票数 |
| --- | ---: |
| 選択肢1 | 0 |
...
**注意事項:**
- プラグイン名のみ大文字小文字を区別しません
- パラメータ（ファイル名、テキストなど）は元の大文字小文字が保持されます
- 例: `#REF(Test.PNG)` → `![Test.PNG](...)`（ファイル名は `Test.PNG` のまま）
...
| `%%取り消し%%` | `~~取り消し~~` | 取り消し線 |
| `%%%下線%%%` | `<u>下線</u>` | 下線 |
| `&br;` または `&br();` | `<br>` | 改行（インライン）<br>単独行の場合は空行を追加 |
| `テキスト~` | `テキスト<br>` | 行末改行 |
| `&size(20){テキスト};` | `<span style="font-size: 20px">テキスト</span>` | 文字サイズ指定 |
| `&color(red){テキスト};` | `<span style="color: red">テキスト</span>` | 文字色指定（新式） |
| `&color(red,yellow){テキスト};` | `<span style="color: red; background-color: yellow">テキスト</span>` | 文字色と背景色指定（新式） |
| `&color(red,テキスト);` | `<span style="color: red">テキスト</span>` | 文字色指定（旧式） |
| `COLOR(red):テキスト` | `<span style="color: red">テキスト</span>` | 文字色指定（通常テキスト） |

**注意事項:**
- **強調マーカーの前後のスペース:** Markdownの正しいレンダリングを保証するため、太字（`**`）、斜体（`*`）、取り消し線（`~~`）のマーカーの前後に自動的にスペースが追加されます
  - 既にスペースがある場合は追加されません
  - 行頭・行末の場合は追加されません
  - 他のマーカー文字（`[`, `]`, `*`, `~` など）の隣の場合は追加されません
  - 例: `これは''太字''です` → `これは **太字** です`
- **強調マーカー内側のスペース:** PukiWikiでは `%% 打消し %%` のようにマーカー内側にスペースがあっても有効ですが、Markdownでは無効になるため、内側のスペース（全角スペース含む）は自動的に除去されます
  - 例: `%% 打消し %%` → `~~打消し~~`
- **ネストした強調マーカー:** 太字と打ち消し線などをネストした場合、マーカー間にスペースは追加されません
  - 例: `''%%太字打消%%''` → `**~~太字打消~~**`
...
- 背景色は省略可能です
- `&color`プラグインには新式（波括弧あり）と旧式（カンマ区切り）の2つの構文があります：
  - 新式: `&color(色){テキスト};` または `&color(色,背景色){テキスト};`
  - 旧式: `&color(色,テキスト);`（背景色指定不可）
  - 両方の構文がサポートされています
...
パス内の特殊文字は、Markdownリンクとして正しく機能するために**必要最小限**のみエンコードされます。日本語などのUnicode文字は可読性向上のためそのまま保持されます。

- **エンコード対象**: `%`, スペース, `()`, `[]`, `:`, `"`, `,`
- **保持される文字**: Unicode文字（日本語など）、英数字、`-_.~`
- 各パス要素を個別にエンコード（パス区切り `/` は保持）
- 相対パスマーカー `.` と `..` はエンコードしない
...
入力: [[ファイル (1)]]
出力: [ファイル (1)](ファイル%20%281%29.md)
```

**リンクテキストのエスケープ:**

リンクテキスト（`[text](url)` の `text` 部分）に含まれる以下の文字はエスケープされ、Markdownパーサーが誤認識しないようにします：

- **エスケープ対象**: `[`, `]`, `\`
- これらの文字はバックスラッシュでエスケープされます（例: `[` → `\[`）

**例:**
```
入力: [[テキスト[括弧]あり]]
出力: [テキスト\[括弧\]あり](テキスト%5B括弧%5D名.md)
      （リンクテキスト部分は \[ \] にエスケープ、URL部分は %5B %5D にエンコード）
...
**注意:** URL部分のエンコードとリンクテキスト部分のエスケープは別々に処理されます。

**リンクテキスト内のインライン書式:**

リンクテキスト内で `&color` や太字などのインライン書式を使用できます。リンク構造を維持したまま、内部の書式が変換されます。

```
入力: [[&color(red){赤文字};>ページ名]]
出力: [<span style="color: red">赤文字</span>](ページ名.md)

入力: [[''太字''>ページ名]]
出力: [**太字**](ページ名.md)

入力: [[&color(blue){青文字};:https://example.com]]
出力: [<span style="color: blue">青文字</span>](https://example.com)
```
...
添付ファイルのパスも内部リンクと同様に**必要最小限**のみエンコードされます。日本語などのUnicode文字は可読性向上のためそのまま保持され、Markdownリンクに問題を引き起こす特殊文字のみエンコードされます。

**例:**
```
ページ名: テスト
入力: #ref(画像 (1).png)
出力: ![画像 (1).png](テスト_attachment_画像%20%281%29.png)
```

**altテキストのエスケープ:**

画像のaltテキストおよびファイルリンクのテキストに含まれる特殊文字は、リンクテキストと同様にエスケープされます。

**例:**
```
入力: #ref(document.pdf,[重要]資料)
出力: [\[重要\]資料](ページ名_attachment_document.pdf)

入力: &ref(image.png,[xxx]タイトル);
出力: ![\[xxx\]タイトル](ページ名_attachment_image.png)
```
...
| PukiWiki | Markdown | 備考 |
|----------|----------|------|
| `#ref(image.png)` | `![image.png](ページ名_attachment_image.png)` | 画像表示（altテキストはファイル名） |
| `#ref(image.png,テキスト)` | `![テキスト](ページ名_attachment_image.png)` | altテキスト指定 |
| `&ref(icon.png);` | `![icon.png](ページ名_attachment_icon.png)` | 画像表示（インライン、altテキストはファイル名） |
| `&ref(icon.png,アイコン);` | `![アイコン](ページ名_attachment_icon.png)` | altテキスト指定（インライン） |

**注意:** altテキストが指定されていない場合、ファイル名がデフォルトのaltテキストとして使用されます。

**例:**
```
ページ名: プロジェクト概要
入力: #ref(screenshot.png)
出力: ![screenshot.png](概要_attachment_screenshot.png)

入力: #ref(画面.png,スクリーンショット)
出力: ![スクリーンショット](概要_attachment_画面.png)
```
...
連続するカンマによる空のパラメータは自動的にフィルタされます（PukiWiki本体の動作に準拠）。

**例:**
```
入力: #ref(document.pdf,,[xxx]タイトル)
出力: [\[xxx\]タイトル](ページ名_attachment_document.pdf)
      （2番目の空パラメータは無視される）

入力: &ref(image.png,,,alt text);
出力: ![alt text](ページ名_attachment_image.png)
      （複数の空パラメータは全て無視される）
```

##### 5.5.1.4 CSV形式のパラメータ解析

PukiWikiの`#ref`と`&ref`は、CSV形式のパラメータ解析をサポートしています。ファイル名にカンマが含まれる場合、ダブルクォートで囲むことができます。

**サポートされる記法:**
- ダブルクォートで囲まれたファイル名: `"file, name.png"`
- エスケープされたダブルクォート: `""` → `"`（ダブルクォート内）
- カンマ区切りのパラメータ

**例:**
```
入力: #ref("ファイル, 名前.png",300x200)
出力: <img src="ページ名_attachment_ファイル%2C%20名前.png" alt="ファイル, 名前.png" width="300" height="200">

入力: #ref("引用符""含む"".png")
出力: ![引用符"含む".png](ページ名_attachment_引用符%22含む%22.png)
```

##### 5.5.1.5 他ページの添付ファイル参照

PukiWikiと同様に、他のページに添付されたファイルを参照できます。相対パス（`../file.png`や`./file.png`）およびページ指定（`PageName/file.png`）をサポートしています。

**サポートされる記法:**
- `PageName/file.png` - 他のページの添付ファイル
- `../file.png` - 親ページの添付ファイル
- `./file.png` - 現在のページの添付ファイル（明示的）

**例:**
```
現在のページ: プロジェクト/タスク
入力: #ref(プロジェクト/概要/diagram.png)
出力: ![diagram.png](概要/概要_attachment_diagram.png)

現在のページ: プロジェクト/タスク
入力: #ref(../header.png)
出力: ![header.png](../プロジェクト_attachment_header.png)

現在のページ: プロジェクト/タスク
入力: #ref(./footer.png)
出力: ![footer.png](タスク_attachment_footer.png)
```

**注意:** 添付ファイルのパスも相対パスとして計算され、必要最小限のみURLエンコードされます。

#### 5.5.2 非画像ファイル参照

非画像ファイルの場合もファイル名がデフォルトのリンクテキストとして使用され、パスは必要最小限のみURLエンコードされます。

| PukiWiki | Markdown | 備考 |
|----------|----------|------|
| `#ref(document.pdf)` | `[document.pdf](ページ名_attachment_document.pdf)` | ファイルリンク（リンクテキストはファイル名） |
...
| `&ref(document.pdf);` | `[document.pdf](ページ名_attachment_document.pdf)` | ファイルリンク（インライン） |

**例:**
```
ページ名: テスト
入力: #ref(資料.pdf)
出力: [資料.pdf](テスト_attachment_資料.pdf)

入力: #ref("仕様書 (最新).xlsx",仕様書)
出力: [仕様書](テスト_attachment_仕様書%20%28最新%29.xlsx)
```
...
- `----` で始まる行は、後続テキストの有無に関わらず水平線として変換されます
- `#hr()` の括弧後のテキストは破棄されます
- `#hr` （括弧なし）の後にテキストがある場合は、プラグインとして認識されません
...
| PukiWiki | Markdown | 備考 |
|----------|----------|------|
| `#br` または `#br()` | `<br>` + 空行 | 改行（プラグイン記法）<br>次の行のMarkdown解析を保証するため空行を追加 |

**注意事項:**
- `#br` または `#br()` の前に空白やタブがある場合は整形済みテキストとして扱われます
- 行末の空白は変換前に削除されます
- `#br` の後に空行が自動的に挿入されます（次の行のMarkdown記法が正しく認識されるようにするため）
...
テーブルセル内で `#ref` ブロックプラグインを使用して、画像やファイルを埋め込むことができます。

**基本的な使い方:**
```
入力:
|#ref(image.png)|#ref(document.pdf,説明)|

出力:
|   |   |
| --- | --- |
| ![image.png](ページ名_attachment_image.png) | [説明](ページ名_attachment_document.pdf) |
```

**動作:**
- セル内容が `#ref(...)` で始まる場合、ブロックプラグインとして処理されます
- 閉じ括弧 `)` の後のテキストは無視されます（他のブロックプラグインと同様の動作）
- `#ref` のすべてのパラメータ（代替テキスト、サイズ指定など）がサポートされます
- セル内容の途中に `#ref(...)` がある場合は、ブロックプラグインとして処理されません

**サイズ指定の例:**
```
入力:
|#ref(image.png,300x200)|説明|h

出力:
| <img src="ページ名_attachment_image.png" alt="image.png" width="300" height="200"> | 説明 |
| --- | --- |
```
...
```
入力:
|BOLD:#ref(image.png)|通常|h

出力:
| **![image.png](ページ名_attachment_image.png)** | 通常 |
| --- | --- |
```

**注意事項:**
- セル内でインライン形式の `&ref(...);` も引き続き使用可能です
- `#ref(...)` がセルの先頭にない場合は、通常のテキストとして扱われます
- 他のブロックプラグイン（`#hr`、`#freeze` など）はセル内では使用できません
...
|----------|--------|------|
| `#back` | `<!-- #back -->` | 戻るリンク |
| `#ls` | `<!-- #ls -->` または `$lsx(./)` | 子ページ一覧<br>`--convert-ls-to-lsx` オプションで GROWI lsx 形式に変換可能 |
| `#ls2` | `<!-- #ls2 -->` または `$lsx(相対パス)` | 子ページ一覧（拡張版）<br>`--convert-ls-to-lsx` オプションで GROWI lsx 形式に変換可能 |
| `#menu` | `<!-- #menu -->` | メニュー表示 |
...
### 5.12 GROWI lsx 形式への変換

`--convert-ls-to-lsx` オプションを使用すると、PukiWiki の `#ls` および `#ls2` プラグインを GROWI の `$lsx` 形式に変換できます。

#### 5.12.1 基本的な変換

**#ls プラグイン（シンプルな子ページ一覧）:**

| PukiWiki | GROWI lsx | 説明 |
|----------|-----------|------|
| `#ls` | `$lsx(./)` | カレントページの子ページ一覧 |
| `#ls()` | `$lsx(./)` | カレントページの子ページ一覧 |
| `#ls(title)` | `<!-- #ls(title) -->`<br>`$lsx(./)` | title オプションは未サポート（HTMLコメントで保持） |

**#ls2 プラグイン（拡張版・パターン指定可能）:**

| PukiWiki | GROWI lsx | 説明 |
|----------|-----------|------|
| `#ls2` | `$lsx(./)` | カレントページの子ページ一覧 |
| `#ls2()` | `$lsx(./)` | カレントページの子ページ一覧 |
| `#ls2(Project/Task)` | `$lsx(./相対パス)` | 指定ページの子ページ一覧（相対パスに変換） |

#### 5.12.2 相対パス変換

PukiWiki のページパスは、カレントページからの相対パスに変換されます：

**例: カレントページが `Project/Task` の場合**

| PukiWiki | GROWI lsx | 説明 |
|----------|-----------|------|
| `#ls2(Project/Task/SubPage)` | `$lsx(./Task/SubPage)` | 子ページへのパス |
| `#ls2(Project/Other)` | `$lsx(./Other)` | 兄弟ページへのパス |
| `#ls2(Project)` | `$lsx(./相対パス)` | 親ページへのパス |

#### 5.12.3 オプションの変換

**サポートされているオプション:**

| PukiWiki オプション | GROWI lsx | 備考 |
|-------------------|-----------|------|
| `reverse` | `reverse=true` | 逆順表示 |

**サポートされていないオプション:**

以下のオプションはGROWI lsxでサポートされていないため、元の行がHTMLコメントとして保持されます：
...
```
入力:
#ls2(Project/Task, reverse)

出力:
$lsx(./Task, reverse=true)
```

```
入力:
#ls2(Project/Task, title)
...
選択肢ラベル内のPukiWiki記法（太字、リンクなど）は、Markdown形式に変換されます。
...
- 選択肢ラベルにはインライン記法の変換が適用される（太字、リンク、装飾など）
- 選択肢の書式: `ラベル[投票数]`
...
PukiWikiの仕様に従い、括弧がある場合は括弧の後のテキストは無視され、行全体がHTMLコメント化されます。括弧がない場合はプラグイン名のみで行が終わる必要があります。

```
入力:
#freeze() 追加テキスト
#contents(depth=2)これは無視される
...
- `&br;` / `&br();` - インライン改行プラグイン（`<br>`タグに変換、単独行の場合は空行を追加）
...
4. パス内の特殊文字（日本語、スペース、括弧など）をURLエンコード
...
1. **カスタムプラグイン**: Markdownに変換できず、コメント化または削除される
2. **InterWiki**: 他のWikiへのリンクは通常のURLリンクとして扱う
3. **ページの別名機能**: PukiWikiのページ別名は未サポート
