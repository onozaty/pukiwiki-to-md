...
**備考:**
- **強調マーカーの前後のスペース:** Markdown の正しいレンダリングを保証するため、太字（`**`）、斜体（`*`）、取り消し線（`~~`）のマーカーの前後に自動的にスペースが追加されます。ただし、既にスペースがある場合や、行頭・行末の場合は追加されません。例: `これは''太字''です` → `これは **太字** です`
...
|----------|----------|------|
| `#br` または `#br()` | `<br>` + 空行 | Markdownの正しい解析を保証するため空行を追加 |
